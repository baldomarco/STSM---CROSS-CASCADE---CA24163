...
plot <- unique(ALS_clean_alive$plotid)

#---- Species proportion -----
sp_prop <- read_excel("C:/P/DMP_CROSS_CASCADE/03_rawdata/02_process_storage/Species_proportion_ALS/sp_prop_plot_ba.xlsx")
...
# plot <- unique(as.character(na.omit(ALS_clean_alive$plotid)))
#--------------------------------------
...
variants.table<-data.frame(#time.event.file=clim_name,
  plot=plot,
  mng.script=paste0(abe.root,"01_abe_bottoms_up_L1_10_unmanaged.js"),
